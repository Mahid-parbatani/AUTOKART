...
function App() {
  ...
  const contextValue = useMemo(
    ...
    [token, email, isMainPage, currentPage, productsList, filters, order, isPaid]
  );
    ...
    <TokenContext.Provider value={contextValue}>
      {token ? (
        isPaid ? (
          ...
          <div style={{ minHeight: '100vh', background: currentPage === 'blackbook' ? '#f8f9fa' : (isMainPage ? backgrounds.main : backgrounds.order), position: 'relative' }}>
            <TopLayout />
            {currentPage === 'blackbook' ? (
              <Blackbook />
            ) : isMainPage ? (
              <div>

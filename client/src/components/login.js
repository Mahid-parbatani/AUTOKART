    ...
    const handleRegister = async (e) => {
      ...
      try {
        ...
        console.log(response.data);
          ...
          if (serverMsg.includes('E11000') || serverMsg.toLowerCase().includes('duplicate')) {
    ...
    return (
        <form onSubmit={handleLogin} style={styles.formLayout}>
          <div style={styles.brandRow}>
            <svg width="36" height="36" viewBox="0 0 64 64" aria-hidden="true" style={{marginRight: '10px'}}>
            ...
            <h2 style={{ fontSize: '2.2em', color: '#102a43', margin: 0, letterSpacing: '0.5px' }}>AUTOKART</h2>
          </div>
          
          <p style={styles.tagline}>Find and order quality used auto parts with ease. Log in or create an account to start your order.</p>
          <input
            type="email"
            placeholder="Email"
            value={email}
            ...
            autoComplete="off"
            style={{ ...styles.textField, marginTop: '24px' }}
          />
          <input
            type="password"
            ...
            style={{ ...styles.textField, marginTop: '12px' }}
          />
          <div style={styles.buttonRow}>
            ...
            <button type="button" onClick={() => { setEmailState(''); setPassword(''); setMessage(''); }} style={{ ...styles.button, ...styles.secondaryButton }}>Clear</button>
          </div>
          {message ? <div style={{ color: '#444', marginTop: '10px' }}>{message}</div> : null}
          {/* Socials (fixed bottom-right) and footer */}
          ...
          <div style={styles.footer}>Terms and Conditions © 2025</div>
        </form>
      );
